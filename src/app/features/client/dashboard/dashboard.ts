...
export default class DashboardComponent implements OnInit {
  ...
  recentInvoices = computed(() => {
    const sorted = [...this.invoices()].sort((a, b) => new Date(b.invoiceDate).getTime() - new Date(a.invoiceDate).getTime());
  ...
  toggleDevice(idDevice: number): void {
    ...
    if (current.has(idDevice)) current.delete(idDevice);
    else current.add(idDevice);
  ...
  get daysLeft(): number { const abo = this.abonnement(); return abo ? Math.max(0, abo.joursRestants) : 0; }
  ...
  get progressColor(): string {
    const map: Record<string, string> = { none: '#9ca3af', pending: '#D97706', active: '#0D9488', warning: '#D97706', danger: '#DC2626', expired: '#7f1d1d' };
    return map[this.subscriptionState];
  }
  get urgencyBg(): string {
    const map: Record<string, string> = {
      none: 'rgba(156,163,175,0.06)', pending: 'rgba(245,158,11,0.05)',
      active: 'rgba(13,148,136,0.05)', warning: 'rgba(217,119,6,0.05)',
      danger: 'rgba(220,38,38,0.05)', expired: 'rgba(127,29,29,0.06)'
    };
    return map[this.subscriptionState];
  }
  get expiryDateStr(): string { const abo = this.abonnement(); return abo ? new Date(abo.endDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'; }
  ...
  get planLabel(): string { const abo = this.abonnement(); return abo ? (abo.dureeLabel ?? `${abo.dureeMois} Month Plan`) : 'No Active Subscription'; }
  get statusChipLabel(): string {
    const map: Record<string, string> = { none: 'NO PLAN', pending: 'PENDING PAYMENT', active: 'ACTIVE', warning: 'EXPIRING SOON', danger: 'CRITICAL', expired: 'EXPIRED' };
    return map[this.subscriptionState];
  }
  ...
  get graceActive(): boolean { return ['warning', 'danger', 'expired'].includes(this.subscriptionState); }
  get graceDaysLeft(): number { return (this.abonnement() as any)?.graceDaysLeft ?? 10; }
  ...
  get activeDevicesCount(): number { return this.clientDevices.filter(d => ['actif', 'active'].includes((d.status ?? '').toLowerCase())).length; }
  ...
  private continueOpenRenewModal() {
    ...
    const abo = this.abonnement();
    const plansUrl = abo ? `http://localhost:8080/api/payment/plans/${abo.idAbo}` : `http://localhost:8080/api/payment/plans-for-client/${this.currentClientId()}`;
  ...
  private openPlansModal() {
    ...
    this.http.get<PlanOption[]>(plansUrl).subscribe({
      next: (p) => {
        this.plans.set(p);
        if (p.length > 0) this.currentPrixUnitaire.set(p[0].prixUnitaire);
  ...
  async selectPlan(plan: PlanOption) {
    ...
    this.http.post<any>('http://localhost:8080/api/payment/initiate-renewal', {
      ...
      error: (err) => { console.error('Payment initiation error:', err); this.toast.error('Failed to create payment session.'); this.renewLoading.set(false); }
    });
  }

  activateGracePeriod() { if (!this.graceActive) return; this.toast.info(`Grace period gives you ${this.graceDaysLeft} extra days — activation coming soon.`); }
